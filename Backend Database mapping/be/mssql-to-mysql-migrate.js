...
function mapType(type, length, columnName, pkCols) {
  ...
  switch (type.toLowerCase()) {
    ...
    case 'varchar':
    case 'nvarchar':
      if (isPK) return 'VARCHAR(255)'; // PK always VARCHAR(255)
      //if (!length || length > 65535) return 'TEXT';
      if (!length || length <= 0 || length > 65535) return 'TEXT';
      return `VARCHAR(${length})`;
    case 'text':
    case 'ntext':
      return isPK ? 'VARCHAR(255)' : 'TEXT'; // PK → VARCHAR(255)
    default:
...
async function createTable(table, pool, mysqlConn) {
  ...
  const columnDefs = cols.recordset.map(c => {
    ...
    const typeDef = mapType(c.DATA_TYPE, c.CHARACTER_MAXIMUM_LENGTH, c.COLUMN_NAME, pkCols);
    let def = `\`${c.COLUMN_NAME}\` ${typeDef}`;

    // NOT NULL for PK columns
    if (isPK) def += ' NOT NULL';

    // AUTO_INCREMENT only for numeric identity columns
    if (
      c.COLUMN_NAME === identityCol &&
      ['int','bigint','smallint','tinyint'].includes(c.DATA_TYPE.toLowerCase())
    ) {
      def += ' AUTO_INCREMENT';
    }
