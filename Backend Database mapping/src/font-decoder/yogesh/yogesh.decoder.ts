...
// 1️⃣ Tokenizer
// =========================
// Inside src/font-decoder/yogesh/yogesh.decoder.ts

function tokenize(raw: string): Token[] {
  const out: Token[] = [];
  for (const ch of raw) {
    // ✅ FIX: Explicitly preserve spaces
    if (ch === ' ') {
      // You might need to add 'SPACE' to your Token type definition too, 
      // or just treat it as a special consonant placeholder if easier.
      // A safer hack without changing types:
      // Just ignore it here and handle it in the service (Option A)
      continue; 
    }
